...
%LET _34MONTH_NUM = %EVAL(%SYSFUNC(inputn(&pulldate,yymmdd10.))-1034);
%LET _34MONTH = %SYSFUNC(putn(&_34MONTH_NUM,yymmdd10.));
%PUT "&_34MONTH";
...
%PUT "&_1DAY" "&_1MONTH" "&_34MONTH" "&_3YR" "&_5YR" "&_15MONTH" 
	 "&_18MONTH";
...
DATA 
	...
	*** ASSIGN ID MACRO VARIABLES -------------------------------- ***;
	CALL SYMPUT ('RETAIL_ID', 'RetailXS_1.2_2020');
	CALL SYMPUT ('AUTO_ID', 'AUTOXS_1.2_2020');
	CALL SYMPUT ('FB_ID', 'FB_1.2_2020CC');
	...
	*** ASSIGN DATA FILE MACRO VARIABLE -------------------------- ***;
	...
	CALL SYMPUT ('FINALEXPORTFLAGGED', 
		'\\mktg-app01\E\Production\2020\01_January_2020\FBXSCC\FBXS_CC_20200109FLAGGED.txt');
	CALL SYMPUT ('FINALEXPORTDROPPED', 
		'\\mktg-app01\E\Production\2020\01_January_2020\FBXSCC\FBXS_CC_20200109FINAL.txt');
	CALL SYMPUT ('EXPORTMLA', 
		'\\mktg-app01\E\Production\MLA\MLA-INPUT FILES TO WEBSITE\FBCC_20200109.txt');
	CALL SYMPUT ('FINALEXPORTED', 
		'\\mktg-app01\E\Production\2020\01_January_2020\FBXSCC\FBXS_CC_20200109FINAL_HH.cSv');
...
DATA LOAN_PULL; /* FROM LOAN TABLE FOR FB */
	...
	WHERE POFFDATE BETWEEN "&_18MONTH" AND "&_1DAY" & 
		  POCD = "13" & 
...
DATA CON1YR_FL; /* FIND FROM 2 YEARS BACK */
	...
	IF _60 > 2 | _90PLUS > 0 THEN CON1YR_FLAG = "X";
	SS7BRSTATE = CATS(SSNO1_RT7, SUBSTR(OWNBR, 1, 2));
...
DATA TEMP;
	...
	*** IF CD IS GREATER THAN 1-29 DAYS LATE, SET CD30 TO 1 ------ ***;
	...
	IF AGE < 4 THEN DO;
		IF CD > 2 THEN RECENT3 = 1; /* 30-59S IN LAST 3 MONTHS */
		IF CD > 3 THEN RECENT3_60 = 1;
	END;
	...
	ELSE IF 0 <= AGE < 16 THEN DO;
		IF CD > 2 THEN LAST15 = 1; /* 30-59S 7 TO 12 MONTHS AGO */
		IF CD > 3 THEN LAST15_60 = 1;
		IF CD > 4 THEN LAST15_90 = 1;
	END;

	KEEP CLASSTRANSLATION BRACCTNO DELQ1-DELQ12 CD CD30 CD60 AGE2 ATBDT
		 AGE RECENT3 RECENT3_60 RECENT4TO6 RECENT6_60 RECENT6_90 
		 FIRST6_60 FIRST6 RECENT6 LAST15 LAST15_60 LAST15_90;
RUN;
...
PROC SUMMARY 
	...
	VAR DELQ1-DELQ12 RECENT6 LAST12 FIRST6 RECENT6_60 RECENT6_90 
		LAST12_60 FIRST6_60 RECENT3 RECENT4TO6 CD60 CD30 LAST15 
		LAST15_60 LAST15_90;
...
DATA FBDLQ;
	...
	IF LAST15 = . THEN LAST15 = 0;
	IF LAST15_60 = . THEN LAST15_60 = 0;
	IF LAST15_90 = . THEN LAST15_90 = 0;
	IF RECENT6_60 = . THEN RECENT6_60 = 0;
	IF RECENT6_90 = . THEN RECENT6_90 = 0;
	IF FIRST6_60 = . THEN FIRST6_60 = 0;
	IF LAST12_60 = . THEN LAST12_60 = 0;
	IF RECENT3 = . THEN RECENT3 = 0;
	IF RECENT3_60 = . THEN RECENT3_60 = 0;
...
DATA ATB; 
	SET dw.vw_AgedTrialBalance(
		...
		WHERE=(BOM between "&_34MONTH" AND "&_1DAY"));
	BRACCTNO = LoanNumber;
	AGE2 = Age2;
	YEARMONTH = BOM;
RUN;
...
DATA ATB;
	...
	*** IF CD IS GREATER THAN 1-29 DAYS LATE, SET CD30 TO 1 ------ ***;
	...
	IF AGE_OPEN < 4 OR AGE_CLOSED < 4 THEN DO;
		IF CD > 2 THEN RECENT3 = 1; *NOTE 30-59S IN LAST THREE MONTHS;
		IF CD > 3 THEN RECENT3_60 = 1;
	END;
	...
	IF AGE_OPEN < 7 OR AGE_CLOSED < 7 THEN DO;
		...
		IF CD > 4 THEN RECENT6_90 = 1;
	END;
	ELSE IF 6 < AGE < 13 OR 6 < AGE_CLOSED < 13 THEN DO;
		*** NOTE 30-59S FROM 7 TO 15 MONTHS AGO ------------------ ***;
		IF CD > 2 THEN FIRST6 = 1;
		IF CD > 3 THEN FIRST6_60 = 1;
	END;
	ELSE IF 0 <= AGE < 16 OR 0 < AGE_CLOSED < 16 THEN DO;
		*** NOTE 30-59S FROM 7 TO 15 MONTHS AGO ------------------ ***;
		IF CD > 2 THEN LAST15 = 1;
		IF CD > 3 THEN LAST15_60 = 1;
		IF CD > 4 THEN LAST15_90 = 1;
	END;
	KEEP BRACCTNO DELQ1-DELQ15 CD CD30 CD60 AGE2 ATBDT AGE_OPEN
		 AGE_CLOSED RECENT3 RECENT3_60 RECENT4TO6 RECENT6_60 RECENT6_90 
		 FIRST6_60 FIRST6 RECENT6 LAST15 LAST15_60 LAST15_90;
RUN;
...
PROC SUMMARY 
	...
	CLASS BRACCTNO;
	VAR DELQ1-DELQ15 RECENT6 LAST12 FIRST6 RECENT6_60 RECENT6_90 
		LAST12_60 FIRST6_60 RECENT3 RECENT3_60 RECENT4TO6 CD60 CD30 
		LAST15 LAST15_60 LAST15_90;
...
DATA MERGED_L_B2; /* FLAG FOR BAD DLQATB */
	SET MERGED_L_B2;
	IF TIMES30 = "." THEN TIMES30 = 0;
	IF RECENT3 > 1 OR RECENT6 >2 OR LAST15 > 3 OR LAST12_60 > 2 
		THEN DLQ_FLAG = "X";
	IF CAMP_TYPE = "FB" & RECENT3_60 > 0 THEN DLQ_FLAG = "X";
	IF CAMP_TYPE = "FB" & RECENT6_60 > 1 THEN DLQ_FLAG = "X";
	IF CAMP_TYPE = "FB" & LAST12_60 > 2 THEN DLQ_FLAG = "X";
	IF CAMP_TYPE = "FB" & RECENT6_90 > 0 THEN DLQ_FLAG = "X";
	IF CAMP_TYPE = "FB" & LAST15_90 > 1 THEN DLQ_FLAG = "X";
...
FILENAME MLA1
"\\mktg-app01\E\Production\MLA\MLA-Output files FROM WEBSITE\MLA_5_3_FBCC_20200109.txt";
